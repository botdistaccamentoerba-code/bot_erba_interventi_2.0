...
def get_interventi_per_mese(anno, mese):
    conn = sqlite3.connect(DATABASE_NAME)
    c = conn.cursor()
    c.execute('''SELECT * FROM interventi 
                 WHERE strftime('%Y', data_uscita) = ? AND strftime('%m', data_uscita) = ?
                 ORDER BY data_uscita DESC''', (anno, mese))
    result = c.fetchall()
    conn.close()
    return result
    ...
    try:
        c.execute('''INSERT INTO interventi 
                    (rapporto_como, progressivo_como, numero_erba, data_uscita, data_rientro,
                     ...
                     cambio_personale, km_finali, litri_rimanenti)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
...
async def mostra_selezione_vigili(query, context):
    ...
    if not vigili_da_selezionare:
        # Se abbiamo già indirizzo e tipologia (per progressivi > 01), salta direttamente alla data rientro
...
async def gestisci_km_finali(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        context.user_data['fase'] = 'litri_rimanenti'
        
        await update.message.reply_text(
        "⛽ **LITRI RIMANENTI**\n\n"
            "Inserisci i litri rimanenti nel mezzo (solo numeri):"
        )
        
    except ValueError:
        await update.message.reply_text("❌ Valore non valido! Inserisci solo numeri interi:")

async def gestisci_litri_rimanenti(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        litri_rimanenti = int(update.message.text.strip())
        if litri_rimanenti < 0:
            raise ValueError("Valore negativo")
        
        context.user_data['nuovo_intervento']['litri_rimanenti'] = litri_rimanenti
        context.user_data['fase'] = 'data_rientro'
        
        await mostra_selezione_data_rientro(update, context)
        
    except ValueError:
        await update.message.reply_text("❌ Valore non valido! Inserisci solo numeri interi:")

async def mostra_selezione_data_rientro(update, context):
    oggi = datetime.now().strftime('%d/%m/%Y')
    ieri = (datetime.now() - timedelta(days=1)).strftime('%d/%m/%Y')
...
async def mostra_riepilogo(update, context):
    ...
    litri_rimanenti = dati.get('litri_rimanenti', 'Non specificato')
...
⛽ **Litri rimanenti:** {litri_rimanenti}
...
async def gestisci_patente_nautica(update: Update, context: ContextTypes.DEFAULT_TYPE, callback_data: str):
    ...
    await query.edit_message_text(
        "🔗 **SAF**\n\n"
        "Il vigile è SAF (Soccorso in Ambiente Fratturato)?",
        reply_markup=reply_markup
    )
...
async def gestisci_progressivo_modifica(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("⛽ Litri Rimanenti", callback_data="campo_litri_rimanenti")],
        [InlineKeyboardButton("🔙 Annulla", callback_data="annulla_modifica")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    id_int, rapporto, prog, num_erba, data_uscita, data_rientro, mezzo_targa, mezzo_tipo, capo, autista, indirizzo, tipologia, cambio_personale, km_finali, litri_rimanenti, created_at = intervento
    ...
    await update.message.reply_text(
        ...
        f"⛽ Litri rimanenti: {litri_rimanenti or 'Non specificato'}",
        reply_markup=reply_markup
    )
...
async def gestisci_selezione_campo(update: Update, context: ContextTypes.DEFAULT_TYPE, campo: str):
    ...
    messaggi_campi = {
        ...
        'mezzo': "Inserisci la nuova targa del mezzo:",
        'capopartenza': "Inserisci il nuovo capopartenza:",
        'autista': "Inserisci il nuovo autista:",
        'indirizzo': "Inserisci il nuovo indirizzo:",
        'tipologia': "Inserisci la nuova tipologia:",
        'km_finali': "Inserisci i nuovi km finali:",
        'litri_rimanenti': "Inserisci i nuovi litri rimanenti:"
    }
...
async def gestisci_valore_modifica(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        elif campo == 'litri_rimanenti':
            nuovo_valore = int(nuovo_valore)
        
        # Aggiorna il database
        aggiorna_intervento(rapporto, progressivo, {
            ...
            'tipologia': 'tipologia',
            'km_finali': 'km_finali',
            'litri_rimanenti': 'litri_rimanenti'
        }[campo], nuovo_valore)
...
async def esporta_dati(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📊 Esporta per mese", callback_data="export_mese")],
...
async def gestisci_export_anno(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['export_tipo'] = 'anno'
    context.user_data['fase_export'] = 'selezione_anno'
...
async def gestisci_export_mese(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    try:
        await query.answer()
    except BadRequest as e:
    ...
    context.user_data['export_tipo'] = 'mese'
    context.user_data['fase_export'] = 'selezione_anno_mese'
    ...
    keyboard = []
    for anno in anni:
        keyboard.append([InlineKeyboardButton(anno, callback_data=f"export_mese_anno_{anno}")])
...
async def gestisci_export_mese_selezione(update: Update, context: ContextTypes.DEFAULT_TYPE, anno: str):
    query = update.callback_query
    try:
        await query.answer()
    except BadRequest as e:
    ...
    context.user_data['export_anno'] = anno
    context.user_data['fase_export'] = 'selezione_mese'
    ...
    keyboard = []
    for nome_mese, numero_mese in mesi:
        keyboard.append([InlineKeyboardButton(nome_mese, callback_data=f"export_mese_{numero_mese}")])
    
    keyboard.append([InlineKeyboardButton("🔙 Indietro", callback_data="export_mese_indietro")])
...
async def esegui_export(update: Update, context: ContextTypes.DEFAULT_TYPE, tipo: str, anno: str = None, mese: str = None):
    query = update.callback_query
    try:
        await query.answer()
    except BadRequest as e:
    ...
    try:
        if tipo == 'anno':
            interventi = get_interventi_per_rapporto('', anno)  # Empty rapporto to get all for year
            filename_suffix = f"anno_{anno}"
            caption = f"Esportazione dati per l'anno {anno}"
        elif tipo == 'mese':
            interventi = get_interventi_per_mese(anno, mese)
            nome_mese = datetime.strptime(mese, '%m').strftime('%B')
            filename_suffix = f"{anno}_{mese}"
            caption = f"Esportazione dati per {nome_mese} {anno}"
        else:  # tutto
        ...
        writer.writerow([
            ...
            'Indirizzo', 'Tipologia', 'Cambio_Personale', 'Km_Finali', 'Litri_Rimanenti', 'Data_Creazione'
        ])
        
        # Dati
        for intervento in interventi:
            id_int, rapporto, progressivo, num_erba, data_uscita, data_rientro, mezzo_targa, mezzo_tipo, capo, autista, indirizzo, tipologia, cambio_personale, km_finali, litri_rimanenti, created_at, partecipanti = intervento
            ...
            writer.writerow([
                ...
                km_finali or '', litri_rimanenti or '', created_fmt
            ])
...
📊 **GRAFICI TELEGRAM:**
Telegram non supporta nativamente la generazione di grafici, ma puoi:
1. Esportare i dati in CSV e crearli con Excel
2. Usare servizi esterni per generare grafici
3. Visualizzare statistiche testuali dettagliate
"""
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    fase_export = context.user_data.get('fase_export')
    ...
    if fase == 'inserisci_rapporto':
        ...
        return
    elif fase == 'litri_rimanenti':
        await gestisci_litri_rimanenti(update, context)
        return
    ...
    if text == "➕ Nuovo Intervento":
        ...
        for intervento in interventi:
            ...
            litri_info = f" | ⛽{litri_rimanenti}L" if litri_rimanenti else ""
        ...
        if rapporto.isdigit():
            ...
            if not interventi:
                ...
                for intervento in interventi:
                    id_int, rapporto, progressivo, num_erba, data_uscita, data_rientro, mezzo_targa, mezzo_tipo, capo, autista, indirizzo, tipologia, cambio_personale, km_finali, litri_rimanenti, created_at = intervento
                    try:
                        data_uscita_fmt = datetime.strptime(data_uscita, '%Y-%m-%d %H:%M:%S').strftime('%d/%m %H:%M')
                        data_rientro_fmt = datetime.strptime(data_rientro, '%Y-%m-%d %H:%M:%S').strftime('%d/%m %H:%M') if data_rientro else 'In corso'
                    except:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Gestione admin
        ...
        await query.edit_message_text("✏️ Modifica vigile - feature in sviluppo")
    ...
    elif data == "export_mese":
        await gestisci_export_mese(update, context)
    ...
    elif data.startswith("export_mese_anno_"):
        anno = data.replace('export_mese_anno_', '')
        await gestisci_export_mese_selezione(update, context, anno)
    
    elif data.startswith("export_mese_"):
        mese = data.replace('export_mese_', '')
        anno = context.user_data.get('export_anno')
        await esegui_export(update, context, 'mese', anno, mese)
    
    elif data == "export_indietro":
        await esporta_dati(update, context)
    
    elif data == "export_mese_indietro":
        await gestisci_export_mese(update, context)
