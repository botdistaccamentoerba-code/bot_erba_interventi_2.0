...
async def gestisci_via(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # FIX: Chiama correttamente la funzione per mostrare la selezione tipologia
    await mostra_selezione_tipologia_intervento(update, context)

async def mostra_selezione_tipologia_intervento(update, context):
    """Mostra la selezione della tipologia di intervento"""
    keyboard = []
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "➕ Nuovo Intervento":
        ...
        # Reset completo come /start
        await start(update, context)
        await update.message.reply_text("🔄 **Interfaccia resettata!**\n\nTutti i flussi attivi sono stati cancellati. Puoi ora iniziare una nuova operazione.")
        return
