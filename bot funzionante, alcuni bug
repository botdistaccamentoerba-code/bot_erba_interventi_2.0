    ...
    try:
        c.execute('''INSERT INTO interventi 
                    (rapporto_como, progressivo_como, numero_erba, data_uscita, data_rientro,
                     mezzo_targa, mezzo_tipo, capopartenza, autista, indirizzo, tipologia, cambio_personale)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                    (dati['rapporto_como'], dati['progressivo_como'], dati['numero_erba'],
                     ...
                     dati.get('cambio_personale', False)))
...
async def gestisci_scelta_tipo(update: Update, context: ContextTypes.DEFAULT_TYPE, callback_data: str):
    ...
    if callback_data == "tipo_nuovo":
        ...
        # Mostra tutti gli interventi recenti, non solo quelli attivi
        conn = sqlite3.connect(DATABASE_NAME)
        c = conn.cursor()
        c.execute('''SELECT id, rapporto_como, progressivo_como, numero_erba, data_uscita, indirizzo
                     FROM interventi 
                     ORDER BY data_uscita DESC LIMIT 10''')
        interventi_recenti = c.fetchall()
        conn.close()
...
async def mostra_selezione_vigili(query, context):
    ...
    if not vigili_da_selezionare:
        ...
        if context.user_data['nuovo_intervento'].get('indirizzo') and context.user_data['nuovo_intervento'].get('tipologia'):
            ...
            await mostra_selezione_data_rientro(query, context)
        else:
...
async def gestisci_tipologia_intervento(update: Update, context: ContextTypes.DEFAULT_TYPE, callback_data: str):
    ...
    if callback_data == "tipologia_altra":
        ...
        # Se abbiamo già l'indirizzo (per progressivi > 01), salta direttamente alla data rientro
...
async def gestione_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📋 Lista Completa", callback_data="admin_lista_completa")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def gestisci_aggiungi_vigile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif fase == 'grado_patente':
        context.user_data['nuovo_vigile']['grado_patente'] = testo
...
async def gestisci_saf(update: Update, context: ContextTypes.DEFAULT_TYPE, callback_data: str):
    ...
    await query.edit_message_text(
        "🚑 **TPSS**\n\n"
        "Il vigile è TPSS (Tecnico Psico-Socio-Sanitario)?",
        reply_markup=reply_markup
    )
...
async def gestisci_tpss(update: Update, context: ContextTypes.DEFAULT_TYPE, callback_data: str):
    ...
    await query.edit_message_text(
        "🤿 **ATP**\n\n"
        "Il vigile è ATP (Addetto al Trasporto Pazienti)?",
        reply_markup=reply_markup
    )
...
async def mostra_lista_completa(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    try:
        await query.answer()
    except BadRequest as e:
    ...
    messaggio = "📊 **SITUAZIONE COMPLETA**\n\n"
    
    messaggio += "👥 **VIGILI**\n"
    for vigile in vigili:
        id_v, nome, cognome, qualifica, grado, nautica, saf, tpss, atp, attivo = vigile
        status = "🟢" if attivo else "🔴"
        messaggio += f"{status} {cognome} {nome} - {qualifica}\n"
...
async def esporta_dati(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        writer.writerow([
            ...
            'Indirizzo', 'Tipologia', 'Cambio_Personale', 'Data_Creazione'
        ])
        
        # Dati
        for intervento in interventi:
            id_int, rapporto, progressivo, num_erba, data_uscita, data_rientro, mezzo_targa, mezzo_tipo, capo, autista, indirizzo, tipologia, cambio_personale, created_at, partecipanti = intervento
            ...
            writer.writerow([
                ...
                indirizzo, tipologia or '', 'Sì' if cambio_personale else 'No', created_fmt
            ])
        ...
        csv_file.name = f"interventi_export_{datetime.now().strftime('%Y%m%d_%H%M')}.csv"
        ...
        await update.message.reply_text(f"❌ Errore durante l'esportazione: {str(e)}")
...
• ⚙️ Gestione - Gestisci vigili e mezzi
    ...
    if text == "➕ Nuovo Intervento":
        ...
        if rapporto.isdigit():
            ...
            if not interventi:
                ...
                for intervento in interventi:
                    id_int, rapporto, progressivo, num_erba, data_uscita, data_rientro, mezzo_targa, mezzo_tipo, capo, autista, indirizzo, tipologia, cambio_personale, created_at = intervento
                    try:
                        data_uscita_fmt = datetime.strptime(data_uscita, '%Y-%m-%d %H:%M:%S').strftime('%d/%m %H:%M')
                        data_rientro_fmt = datetime.strptime(data_rientro, '%Y-%m-%d %H:%M:%S').strftime('%d/%m %H:%M') if data_rientro else 'In corso'
                    except:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data == "admin_lista_completa":
        await mostra_lista_completa(update, context)
