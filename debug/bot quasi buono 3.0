    ...
    try:
        ...
        for row in reader:
            try:
                ...
                qualifica = row[2]
                grado_patente = row[3]
                patente_nautica = row[4].lower() == 'sì'
                saf = row[5].lower() == 'sì'
                tpss = row[6].lower() == 'sì'
                atp = row[7].lower() == 'sì'
...
# === BACKUP AUTOMATICO ===
async def backup_automatico_interventi():
    """Funzione per backup automatico giornaliero degli interventi"""
    try:
        # Crea file CSV in memoria
        output = StringIO()
        writer = csv.writer(output)
        ...
        # Prendi tutti gli interventi dell'ultimo giorno
        oggi = datetime.now().strftime('%Y-%m-%d')
        interventi = get_interventi_per_anno(datetime.now().year)
...
async def invia_backup_automatico(context: ContextTypes.DEFAULT_TYPE):
    """Invia il backup automatico agli admin"""
    try:
        csv_data = await backup_automatico_interventi()
        if csv_data:
            csv_bytes = csv_data.encode('utf-8')
            csv_file = BytesIO(csv_bytes)
            csv_file.name = f"backup_interventi_{datetime.now().strftime('%Y%m%d')}.csv"
            ...
            for admin_id in ADMIN_IDS:
                try:
                    await context.bot.send_document(
                        ...
                        document=csv_file,
                        filename=csv_file.name,
                        caption="📊 **BACKUP AUTOMATICO GIORNALIERO**\n\nBackup completo del database interventi."
                    )
                    # Reset del buffer
                    csv_file.seek(0)
                except Exception as e:
                    print(f"❌ Errore invio backup a {admin_id}: {e}")
            
            print("✅ Backup automatico inviato agli admin")
    except Exception as e:
        print(f"❌ Errore durante backup automatico: {e}")

async def invia_backup_mensile(context: ContextTypes.DEFAULT_TYPE):
    """Invia il backup mensile di vigili e mezzi agli admin"""
    try:
        # Crea file CSV per vigili e mezzi
        vigili = get_tutti_vigili()
        mezzi = get_tutti_mezzi()
        ...
        csv_bytes = csv_data.encode('utf-8')
        csv_file = BytesIO(csv_bytes)
        csv_file.name = f"backup_vigili_mezzi_{datetime.now().strftime('%Y%m')}.csv"
        ...
        for admin_id in ADMIN_IDS:
            try:
                await context.bot.send_document(
                    ...
                    document=csv_file,
                    filename=csv_file.name,
                    caption="🏠 **BACKUP MENSILE VIGILI E MEZZI**\n\nBackup completo di vigili e mezzi."
                )
                # Reset del buffer
                csv_file.seek(0)
            except Exception as e:
                print(f"❌ Errore invio backup mensile a {admin_id}: {e}")
        
        print("✅ Backup mensile vigili e mezzi inviato agli admin")
    except Exception as e:
        print(f"❌ Errore durante backup mensile: {e}")

# === SCHEDULER BACKUP AUTOMATICO ===
async def scheduled_backup(context: ContextTypes.DEFAULT_TYPE):
    """Funzione schedulata per i backup automatici"""
    now = datetime.now()
    
    # Backup giornaliero alle 23:55
    if now.hour == 23 and now.minute == 55:
        await invia_backup_automatico(context)
    
    # Backup mensile (ultimo giorno del mese alle 23:55)
    next_day = now + timedelta(days=1)
    if now.day != next_day.day and now.hour == 23 and now.minute == 55:
        await invia_backup_mensile(context)
...
async def importa_vigili_csv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        "Nome,Cognome,Qualifica,Grado Patente,Patente Nautica (Sì/No),SAF (Sì/No),TPSS (Sì/No),ATP (Sì/No)\n\n"
        "Esempio:\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "➕ Nuovo Intervento":
        # RESET di eventuali stati precedentes
        for key in ['nuovo_intervento', 'fase', 'vigili_da_selezionare', 'vigili_selezionati']:
            if key in context.user_data:
                del context.user_data[key]
...
def main():
    ...
    # Aggiungi job per backup automatico
    job_queue = application.job_queue
    if job_queue:
        job_queue.run_repeating(scheduled_backup, interval=60, first=10)  # Controlla ogni minuto
    ...
    print("📊 Backup automatici giornalieri e mensili attivati!")
