...
def crea_tastiera_fisica(user_id):
    ...
    tastiera = [
        ...
        [KeyboardButton("➕ Nuova Tipologia"), KeyboardButton("🆘 Help")]
    ]
...
async def avvia_nuova_tipologia(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if not is_user_approved(user_id):
        return

    context.user_data['nuova_tipologia'] = True
    await update.message.reply_text(
        "📝 **NUOVA TIPOLOGIA INTERVENTO**\n\n"
        "Inserisci il nome della nuova tipologia di intervento:"
    )

async def gestisci_nuova_tipologia(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.user_data.get('nuova_tipologia'):
        return
    
    nuova_tipologia = update.message.text.strip()
    
    if not nuova_tipologia:
    ...
    keyboard = [
        [
            InlineKeyboardButton("✅ Conferma", callback_data=f"conferma_tipologia_{nuova_tipologia}"),
            InlineKeyboardButton("❌ Annulla", callback_data="annulla_tipologia")
        ]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await update.message.reply_text(
        f"⚠️ **CONFERMA NUOVA TIPOLOGIA**\n\n"
        f"Stai per aggiungere la tipologia:\n**{nuova_tipologia}**\n\n"
        f"Confermi l'aggiunta?",
        reply_markup=reply_markup
    )

async def conferma_nuova_tipologia(update: Update, context: ContextTypes.DEFAULT_TYPE, tipologia: str):
    ...
    if aggiungi_tipologia_intervento(tipologia):
        await query.edit_message_text(
            f"✅ **NUOVA TIPOLOGIA AGGIUNTA!**\n\n"
            f"Tipologia: **{tipologia}**\n\n"
            f"La nuova tipologia è ora disponibile per gli interventi."
        )
    else:
        await query.edit_message_text(
            f"❌ **TIPOLOGIA GIA' ESISTENTE**\n\n"
            f"La tipologia **{tipologia}** è già presente nell'elenco."
        )
    
    # Cleanup
    if 'nuova_tipologia' in context.user_data:
        del context.user_data['nuova_tipologia']
...
async def seleziona_vigile_modifica(update: Update, context: ContextTypes.DEFAULT_TYPE, vigile_id: int):
    ...
    # FIX: Rimossi Nome e Cognome dall'elenco, solo campi modificabili
    keyboard = [
...
async def gestisci_campo_vigile(update: Update, context: ContextTypes.DEFAULT_TYPE, campo: str):
    ...
    if campo in ['nautica', 'saf', 'tpss', 'atp', 'stato']:
        ...
        # Cleanup
        if 'modifica_vigile' in context.user_data:
            del context.user_data['modifica_vigile']
...
async def gestisci_selezione_campo(update: Update, context: ContextTypes.DEFAULT_TYPE, campo: str):
    ...
    if campo in ['mezzo', 'capopartenza', 'autista']:
        ...
        # FIX: Solo tipologie predefinite, niente "aggiungi"
        keyboard = []
        for tipologia in TIPOLOGIE_INTERVENTO:
...
• ➕ Nuova Tipologia - Aggiungi nuova tipologia intervento
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('nuova_tipologia'):
        await gestisci_nuova_tipologia(update, context)
        return
    ...
    elif text == "➕ Nuova Tipologia":
        await avvia_nuova_tipologia(update, context)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Gestione nuova tipologia
    elif data.startswith("conferma_tipologia_"):
        tipologia = data.replace('conferma_tipologia_', '')
        await conferma_nuova_tipologia(update, context, tipologia)
    
    elif data == "annulla_tipologia":
        await query.edit_message_text("❌ Aggiunta tipologia annullata.")
        if 'nuova_tipologia' in context.user_data:
            del context.user_data['nuova_tipologia']
    ...
    # Gestione modifica vigile
        ...
        if data == "modifica_vigile":
            ...
            if len(parts) == 2:
                ...
                if parts[1] in ['qualifica', 'grado', 'nautica', 'saf', 'tpss', 'atp', 'stato']:
