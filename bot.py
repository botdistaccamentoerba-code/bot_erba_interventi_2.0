...
# === NUOVO INTERVENTO - FLUSSO COMPLETO RISCIRITTO ===
async def avvia_nuovo_intervento(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for key in list(context.user_data.keys()):
        if key.startswith(('nuovo_intervento', 'fase', 'vigili_', 'modifica_')):
            del context.user_data[key]
...
async def gestisci_collega_intervento(update: Update, context: ContextTypes.DEFAULT_TYPE, intervento_id: int):
    ...
    if intervento:
        ...
        await avvia_inserimento_data_uscita(query, context)

async def gestisci_rapporto_como(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get('fase') != 'inserisci_rapporto':
        return
    ...
    await avvia_inserimento_data_uscita(update.message, context)

async def avvia_inserimento_data_uscita(message_obj, context):
...
async def gestisci_ora_uscita(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get('fase') != 'ora_uscita':
        return
    ...
    try:
        ...
        await avvia_inserimento_data_rientro(update.message, context)
        
    except ValueError:
        await update.message.reply_text("❌ Formato ora non valido! Inserisci 4 cifre (es: 1423 per 14:23):")

async def avvia_inserimento_data_rientro(message_obj, context):
...
async def gestisci_ora_rientro(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get('fase') != 'ora_rientro':
        return
    ...
    try:
        ...
        await avvia_selezione_mezzo(update.message, context)
        
    except ValueError:
        await update.message.reply_text("❌ Formato ora non valido! Inserisci 4 cifre (es: 1630 per 16:30):")

async def avvia_selezione_mezzo(message_obj, context):
...
async def gestisci_selezione_mezzo(update: Update, context: ContextTypes.DEFAULT_TYPE, callback_data: str):
    ...
    if progressivo in ['02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12']:
        await avvia_cambio_personale(query, context)
    else:
        context.user_data['nuovo_intervento']['cambio_personale'] = False
        await avvia_selezione_capopartenza(query, context)

async def avvia_cambio_personale(query, context):
...
async def avvia_selezione_capopartenza(query, context):
...
async def gestisci_selezione_capopartenza(update: Update, context: ContextTypes.DEFAULT_TYPE, callback_data: str):
    ...
    await avvia_selezione_autista(query, context)

async def avvia_selezione_autista(query, context):
...
async def gestisci_selezione_autista(update: Update, context: ContextTypes.DEFAULT_TYPE, callback_data: str):
    ...
    # Inizializza lista partecipanti
    ...
    await avvia_selezione_vigili(query, context)

async def avvia_selezione_vigili(query, context):
...
async def mostra_selezione_vigili(query, context):
    ...
    if not vigili_da_selezionare:
        await avvia_inserimento_comune(query, context)
        return
...
async def avvia_inserimento_comune(query, context):
...
async def gestisci_comune(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get('fase') != 'inserisci_comune':
        return
...
async def gestisci_via(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get('fase') != 'inserisci_via':
        return
    ...
    await avvia_selezione_tipologia(update.message, context)

async def avvia_selezione_tipologia(message_obj, context):
...
async def gestisci_tipologia_intervento(update: Update, context: ContextTypes.DEFAULT_TYPE, callback_data: str):
    ...
    if callback_data == "tipologia_altro":
        ...
        context.user_data['nuovo_intervento']['tipologia'] = tipologia
        await avvia_inserimento_km_finali(query, context)

async def gestisci_tipologia_personalizzata(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get('fase') != 'inserisci_tipologia_personalizzata':
        return
    
    tipologia = update.message.text.strip()
    context.user_data['nuovo_intervento']['tipologia'] = tipologia
    await avvia_inserimento_km_finali(update.message, context)

async def avvia_inserimento_km_finali(message_obj, context):
...
async def gestisci_km_finali(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get('fase') != 'km_finali':
        return
    ...
    try:
        ...
        await avvia_inserimento_litri_riforniti(update.message, context)
        
    except ValueError:
        await update.message.reply_text("❌ Valore non valido! Inserisci solo numeri interi:")

async def avvia_inserimento_litri_riforniti(message_obj, context):
...
async def gestisci_litri_riforniti(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get('fase') != 'litri_riforniti':
        return
    ...
    try:
        ...
        await mostra_riepilogo_intervento(update.message, context)
        
    except ValueError:
        await update.message.reply_text("❌ Valore non valido! Inserisci solo numeri interi:")

async def mostra_riepilogo_intervento(message_obj, context):
...
"""
    ...
    await message_obj.reply_text(riepilogo, reply_markup=reply_markup)
...
async def conferma_intervento(update: Update, context: ContextTypes.DEFAULT_TYPE, callback_data: str):
    ...
    for key in ['nuovo_intervento', 'fase', 'vigili_da_selezionare']:
...
async def reset_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Messaggio di conferma reset
    await update.message.reply_text("🔄 **Ripristino fatto, tutto PASS...**\n\nTutti i flussi attivi sono stati cancellati. Il bot è stato riavviato.")
