...
async def gestisci_import_utenti(update: Update, context: ContextTypes.DEFAULT_TYPE, reader):
    ...
    for row in reader:
        try:
            if len(row) < 4:  # Controlla che ci siano abbastanza colonne
                error_count += 1
                continue
            
            # MAPPATURA colonne utenti:
            # 0: user_id, 1: username, 2: nome, 3: telefono, 4: ruolo (opzionale)
...
async def esegui_export_utenti(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Crea file CSV per utenti approvati
...
async def gestisci_progressivo_modifica(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for intervento in interventi_anno:
        if len(intervento) >= 4:  # Assicurati che ci siano abbastanza colonne
            rapporto_db = intervento[1]  # rapporto_como è al 2° campo
            progressivo_db = intervento[2]  # progressivo_como è al 3° campo
