...
# Funzioni di utilità per il mapping - VERSIONE CORRETTA
...
# === SISTEMA DI SELEZIONE TIPOLOGIA PAGINATO CON MAPPING CORRETTO ===
def crea_tastiera_tipologie_paginata(page=0, items_per_page=8):
    ...
    for tipologia in tipologie_pagina:
        display_name = TIPOLOGIE_MAPPING[tipologia][0]  # Nome breve per display
        callback_breve = TIPOLOGIE_MAPPING[tipologia][1]  # Callback breve
...
# === GESTIONE TIPOLOGIA NEL FLUSSO NUOVO INTERVENTO - CORRETTA ===
async def gestisci_tipologia_intervento(update: Update, context: ContextTypes.DEFAULT_TYPE, callback_data: str):
    ...
    if callback_data.startswith("tipopage_"):
        ...
        # Selezione tipologia dalla lista - USO CORRETTO DEL MAPPING
        ...
        if tipologia_completa:
            ...
            display_name = get_display_name(tipologia_completa)
...
# === GESTIONE TIPOLOGIA NEL FLUSSO MODIFICA INTERVENTO - CORRETTA ===
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("tipo_"):
    ...
    elif data.startswith("tipologia_") or data.startswith("tipopage_"):
        fase = context.user_data.get('fase')
        fase_modifica = context.user_data.get('fase_modifica')
        ...
        if fase == 'tipologia_intervento':
            ...
            # Fallback - prova a gestire comunque
            await gestisci_tipologia_intervento(update, context, data)
