...
# Tipi mezzi predefiniti con ordine specifico
TIPI_MEZZO_ORDINATI = [
    ('26613', 'APS TLF3'),
    ('24674', 'ABP Daf'),
    ('26690', 'A/TRID ML120E'),
...
def init_db():
    ...
    # Inserisci dati iniziali mezzi nell'ordine specifico
    for targa, tipo in TIPI_MEZZO_ORDINATI:
...
def get_mezzi_attivi_ordinati():
    """Restituisce i mezzi attivi nell'ordine specifico desiderato"""
    conn = sqlite3.connect(DATABASE_NAME)
    c = conn.cursor()
    c.execute('''SELECT targa, tipo FROM mezzi WHERE attivo = 1''')
    mezzi_db = c.fetchall()
    conn.close()
    
    # Crea un dizionario per lookup veloce
    mezzi_dict = {targa: tipo for targa, tipo in mezzi_db}
    
    # Ordina secondo l'ordine specificato, mantenendo solo quelli attivi
    mezzi_ordinati = []
    for targa, tipo in TIPI_MEZZO_ORDINATI:
        if targa in mezzi_dict:
            mezzi_ordinati.append((targa, tipo))
    
    # Aggiungi eventuali mezzi non presenti nella lista ordinata
    for targa, tipo in mezzi_db:
        if (targa, tipo) not in mezzi_ordinati:
            mezzi_ordinati.append((targa, tipo))
    
    return mezzi_ordinati
...
def get_tipi_mezzo():
    ...
    tipi_predefiniti = [tipo for _, tipo in TIPI_MEZZO_ORDINATI]
    for tipo in tipi_predefiniti:
...
def crea_tastiera_fisica(user_id):
    ...
    tastiera = [
        ...
        [KeyboardButton("📤 Estrazione Dati"), KeyboardButton("🆘 Help"), KeyboardButton("🔄 Reset")]
    ]
...
async def gestisci_ora_rientro(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # USO LA NUOVA FUNZIONE CON MEZZI ORDINATI
        mezzi = get_mezzi_attivi_ordinati()
        keyboard = []
...
• 🔄 Reset - Tasto di emergenza per uscire da qualsiasi flusso
...
🔄 **TASTO RESET:**
Usa il tasto "🔄 Reset" per uscire da qualsiasi flusso di inserimento dati in caso di errore o per cambiare funzione.
"""
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # GESTIONE TASTO RESET - PRIORITÀ ASSOLUTA
    if text == "🔄 Reset":
        ...
        await update.message.reply_text(
            "🔄 **RESET COMPLETO**\n\n"
            "Tutti i flussi di inserimento sono stati cancellati.\n"
            "Puoi ora utilizzare normalmente il bot.",
            reply_markup=crea_tastiera_fisica(user_id)
        )
        return
