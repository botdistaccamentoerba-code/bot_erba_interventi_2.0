...
TIPOLOGIE_INTERVENTO = [
    ...
    "Fuoriuscita gas", "Recupero animali", "Altro"
]
...
async def gestisci_scelta_tipo(update: Update, context: ContextTypes.DEFAULT_TYPE, callback_data: str):
    ...
    if callback_data == "tipo_nuovo":
        ...
        # FIX: Mostra tutti gli interventi recenti, non solo quelli attivi
        conn = sqlite3.connect(DATABASE_NAME)
        c = conn.cursor()
        c.execute('''SELECT id, rapporto_como, progressivo_como, numero_erba, data_uscita, indirizzo
                     FROM interventi 
...
async def mostra_selezione_data_rientro(query, context):
    oggi = datetime.now().strftime('%d/%m/%Y')
    ieri = (datetime.now() - timedelta(days=1)).strftime('%d/%m/%Y')
...
async def gestisci_aggiungi_vigile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if fase == 'nome':
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("VVF", callback_data="qualifica_VVF")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
...
async def gestisci_qualifica_vigile(update: Update, context: ContextTypes.DEFAULT_TYPE, qualifica: str):
    ...
    await query.edit_message_text(
        "📝 **GRADO PATENTE**\n\n"
        "Inserisci il grado della patente terrestre (es: I, II, III, IIIE):"
    )
...
async def gestisci_aggiungi_mezzo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if fase == 'targa':
        ...
        keyboard = [
            [InlineKeyboardButton("APS", callback_data="tipo_APS")],
            [InlineKeyboardButton("ABP", callback_data="tipo_ABP")],
            [InlineKeyboardButton("AS", callback_data="tipo_AS")],
            [InlineKeyboardButton("AU", callback_data="tipo_AU")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
...
async def esporta_dati(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        writer.writerow([
            ...
            'Indirizzo', 'Tipologia', 'Data_Creazione'
        ])
        ...
        for intervento in interventi:
            ...
            writer.writerow([
                ...
                indirizzo, tipologia or '', created_fmt
            ])
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if fase == 'inserisci_rapporto':
        ...
        return
    elif fase_vigile in ['nome', 'cognome', 'grado_patente']:
