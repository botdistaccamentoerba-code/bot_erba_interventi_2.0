...
async def gestisci_import_interventi(update: Update, context: ContextTypes.DEFAULT_TYPE, reader):
    ...
    for row_num, row in enumerate(reader, start=2):
        try:
            ...
            if len(row) < 16:  # CAMBIATO: da 17 a 16
                error_count += 1
            ...
            dati = {
                ...
                'comune': row[10] if len(row) > 10 else '',  # CAMBIATO: da 11 a 10
                'via': row[11] if len(row) > 11 else '',     # CAMBIATO: da 12 a 11
                'indirizzo': '',  # CAMBIATO: indirizzo vuoto, calcolato da comune e via
                'tipologia': row[12] if len(row) > 12 else '',  # CAMBIATO: da 13 a 12
                'cambio_personale': row[13].lower() in ['sì', 'si', '1', 'true', 'vero'] if len(row) > 13 else False,  # CAMBIATO: da 14 a 13
                'km_finali': int(row[14]) if len(row) > 14 and row[14] and row[14].isdigit() else None,  # CAMBIATO: da 15 a 14
                'litri_riforniti': int(row[15]) if len(row) > 15 and row[15] and row[15].isdigit() else None  # CAMBIATO: da 16 a 15
            }
