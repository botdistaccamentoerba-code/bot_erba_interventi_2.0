...
def aggiungi_tipologia_intervento(nuova_tipologia):
    """Aggiunge una nuova tipologia di intervento alla lista"""
    if nuova_tipologia and nuova_tipologia not in TIPOLOGIE_INTERVENTO:
        TIPOLOGIE_INTERVENTO.append(nuova_tipologia)
        return True
    return False

# === IMPORT/EXPORT CSV FUNCTIONS ===
async def importa_csv_interventi(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for row in reader:
            try:
                ...
                # Controllo se l'intervento esiste già
                existing_intervento = get_intervento_by_rapporto(rapporto_como, progressivo_como)
                if existing_intervento:
                    skipped_count += 1
                    continue
        ...
        await update.message.reply_text(
            ...
            f"• ⏭️ Saltati (esistenti): {skipped_count}\n"
...
async def importa_csv_vigili(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if not is_admin(user_id):
        await update.message.reply_text("❌ Solo gli amministratori possono importare dati.")
        return
    ...
    try:
        ...
        for row in reader:
            try:
                ...
                qualifica = row[2]
                grado_patente = row[3]
                patente_nautica = row[4] == 'Sì'
                saf = row[5] == 'Sì'
                tpss = row[6] == 'Sì'
                atp = row[7] == 'Sì'
                
                # Controlla se il vigile esiste già
                conn = sqlite3.connect(DATABASE_NAME)
                c = conn.cursor()
                ...
                if existing_vigile:
                    ...
                    c.execute('''UPDATE vigili SET 
                                ...
                                saf = ?, tpss = ?, atp = ?
                                WHERE id = ?''',
                                (qualifica, grado_patente, patente_nautica, saf, tpss, atp, existing_vigile[0]))
                else:
        ...
        await update.message.reply_text(
            ...
            f"📊 **Risultati:**\n"
            f"• ✅ Record importati/aggiornati: {imported_count}\n"
            f"• ❌ Errori: {error_count}\n\n"
            f"I dati sono stati aggiunti al database."
        )
...
async def importa_csv_mezzi(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if not is_admin(user_id):
        await update.message.reply_text("❌ Solo gli amministratori possono importare dati.")
        return
    ...
    try:
        ...
        for row in reader:
            try:
                if len(row) < 2:  # Controlla che ci siano abbastanza colonne
                    error_count += 1
                    continue
        ...
        await update.message.reply_text(
            f"✅ **IMPORTAZIONE MEZZI COMPLETATA**\n\n"
            f"📊 **Risultati:**\n"
            f"• ✅ Record importati/aggiornati: {imported_count}\n"
            f"• ❌ Errori: {error_count}\n\n"
            f"I dati sono stati aggiunti al database."
...
def esporta_csv_interventi():
    """Esporta tutti gli interventi in formato CSV"""
    ...
    for intervento in interventi:
        if len(intervento) >= 18:  # Assicurati che ci siano abbastanza colonne
            ...
            writer.writerow([
                ...
                comune or '', via or '', indirizzo or '', tipologia or '', 'Sì' if cambio_personale else 'No', 
...
def esporta_csv_vigili():
    """Esporta tutti i vigili in formato CSV"""
    ...
    writer.writerow([
        'Nome', 'Cognome', 'Qualifica', 'Grado_Patente', 'Patente_Nautica', 'SAF', 'TPSS', 'ATP', 'Stato'
    ])
...
def esporta_csv_mezzi():
    """Esporta tutti i mezzi in formato CSV"""
    ...
    for mezzo in mezzi:
        ...
        writer.writerow([
            targa, tipo, 'Attivo' if attivo else 'Non attivo'
        ])
...
async def invia_backup_automatico():
    """Invia backup automatico agli admin"""
    try:
        # Esporta i dati
        csv_interventi = esporta_csv_interventi()
        csv_interventi_bytes = csv_interventi.encode('utf-8')
        csv_interventi_file = BytesIO(csv_interventi_bytes)
        csv_interventi_file.name = f"backup_interventi_{datetime.now().strftime('%Y%m%d')}.csv"
        
        # Controlla se è fine mese
        oggi = datetime.now()
        domani = oggi + timedelta(days=1)
        is_fine_mese = oggi.month != domani.month
        
        if is_fine_mese:
            # Invia anche vigili e mezzi
            csv_vigili = esporta_csv_vigili()
            csv_vigili_bytes = csv_vigili.encode('utf-8')
            csv_vigili_file = BytesIO(csv_vigili_bytes)
            csv_vigili_file.name = f"backup_vigili_{datetime.now().strftime('%Y%m')}.csv"
            
            csv_mezzi = esporta_csv_mezzi()
            csv_mezzi_bytes = csv_mezzi.encode('utf-8')
            csv_mezzi_file = BytesIO(csv_mezzi_bytes)
            csv_mezzi_file.name = f"backup_mezzi_{datetime.now().strftime('%Y%m')}.csv"
            ...
            try:
                if is_fine_mese:
                    await application.bot.send_message(
                        admin_id,
                        f"📊 **BACKUP AUTOMATICO FINE MESE**\n"
                        f"Data: {oggi.strftime('%d/%m/%Y %H:%M')}\n"
                        f"File inviati: Interventi, Vigili, Mezzi"
                    )
                    await application.bot.send_document(
                        chat_id=admin_id,
                        document=csv_interventi_file,
                        filename=csv_interventi_file.name,
                        caption="📤 Backup interventi giornaliero"
                    )
                    await application.bot.send_document(
                        chat_id=admin_id,
                        document=csv_vigili_file,
                        filename=csv_vigili_file.name,
                        caption="👥 Backup vigili mensile"
                    )
                    await application.bot.send_document(
                        chat_id=admin_id,
                        document=csv_mezzi_file,
                        filename=csv_mezzi_file.name,
                        caption="🚒 Backup mezzi mensile"
                    )
                else:
                    await application.bot.send_message(
                        admin_id,
                        f"📊 **BACKUP AUTOMATICO GIORNALIERO**\n"
                        f"Data: {oggi.strftime('%d/%m/%Y %H:%M')}"
                    )
                    await application.bot.send_document(
                        chat_id=admin_id,
                ...
                print(f"❌ Errore invio backup a admin {admin_id}: {e}")
        
        print(f"✅ Backup automatico inviato agli admin - Fine mese: {is_fine_mese}")
        
    except Exception as e:
        print(f"❌ Errore durante backup automatico: {e}")

async def scheduler_backup_automatico():
    """Scheduler per backup automatici"""
    print("🔄 Scheduler backup automatico avviato")
    ...
    while True:
        try:
            ...
            if now.hour == 23 and now.minute == 55:
                print("🕐 Invio backup automatico...")
                await invia_backup_automatico()
                # Aspetta 65 minuti per evitare duplicati
                await asyncio.sleep(3900)
            else:
                # Controlla ogni minuto
                await asyncio.sleep(60)
        except Exception as e:
            print(f"❌ Errore scheduler backup: {e}")
            await asyncio.sleep(60)
...
def crea_tastiera_fisica(user_id):
    ...
    tastiera = [
        ...
        [KeyboardButton("➕ Nuovo Tipo Intervento"), KeyboardButton("🆘 Help")]
    ]
...
async def gestisci_file_csv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Determina il tipo di importazione in base al nome del file
    filename = document.file_name.lower()
    
    if 'vigili' in filename:
        await importa_csv_vigili(update, context)
    elif 'mezzi' in filename:
        await importa_csv_mezzi(update, context)
    else:
        await importa_csv_interventi(update, context)

# === NUOVO TIPO INTERVENTO ===
async def avvia_nuovo_tipo_intervento(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if not is_user_approved(user_id):
        return

    # RESET di eventuali stati precedenti per evitare conflitti
    for key in ['nuovo_tipo_intervento', 'nuova_tipologia']:
        if key in context.user_data:
            del context.user_data[key]
    
    context.user_data['nuovo_tipo_intervento'] = True
    await update.message.reply_text(
        "📝 **NUOVO TIPO INTERVENTO**\n\n"
        "Inserisci il nome del nuovo tipo di intervento:"
    )

async def gestisci_nuovo_tipo_intervento(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.user_data.get('nuovo_tipo_intervento'):
        return
    
    nuovo_tipo = update.message.text.strip()
    
    if not nuovo_tipo:
        await update.message.reply_text("❌ Tipo intervento non valido! Riprova:")
        return
    
    # Conferma prima di aggiungere
    keyboard = [
        [
            InlineKeyboardButton("✅ Conferma", callback_data=f"conferma_tipo_{nuovo_tipo}"),
            InlineKeyboardButton("❌ Annulla", callback_data="annulla_tipo")
        ]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await update.message.reply_text(
        f"⚠️ **CONFERMA NUOVO TIPO INTERVENTO**\n\n"
        f"Stai per aggiungere il tipo:\n**{nuovo_tipo}**\n\n"
        f"Confermi l'aggiunta?",
        reply_markup=reply_markup
    )

async def conferma_nuovo_tipo_intervento(update: Update, context: ContextTypes.DEFAULT_TYPE, tipo: str):
    ...
    if aggiungi_tipologia_intervento(tipo):
        await query.edit_message_text(
            f"✅ **NUOVO TIPO INTERVENTO AGGIUNTO!**\n\n"
            f"Tipo: **{tipo}**\n\n"
            f"Il nuovo tipo è ora disponibile per gli interventi."
        )
    else:
        await query.edit_message_text(
            f"❌ **TIPO INTERVENTO GIA' ESISTENTE**\n\n"
            f"Il tipo **{tipo}** è già presente nell'elenco."
        )
    
    # Cleanup
    if 'nuovo_tipo_intervento' in context.user_data:
        del context.user_data['nuovo_tipo_intervento']
...
async def gestisci_collega_intervento(update: Update, context: ContextTypes.DEFAULT_TYPE, intervento_id: int):
    ...
    conn = sqlite3.connect(DATABASE_NAME)
    c = conn.cursor()
    c.execute('''SELECT rapporto_como, numero_erba, comune, via, indirizzo, tipologia FROM interventi WHERE id = ?''', (intervento_id,))
    intervento = c.fetchone()
    conn.close()
    
    if intervento:
        rapporto_como, numero_erba, comune, via, indirizzo, tipologia = intervento
        ...
        if progressivo_como != "01":
            if comune:
                context.user_data['nuovo_intervento']['comune'] = comune
            if via:
        ...
        if progressivo_como != "01":
            if comune or via:
                messaggio += f"📍 Indirizzo: {comune or ''} {via or ''}\n"
            if tipologia:
...
async def mostra_selezione_vigili(query, context):
    ...
    if not vigili_da_selezionare:
        ...
        if context.user_data['nuovo_intervento'].get('comune') and context.user_data['nuovo_intervento'].get('tipologia'):
...
async def gestisci_via(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Ricostruisci l'indirizzo completo
    comune = context.user_data['nuovo_intervento'].get('comune', '')
    indirizzo_completo = f"{comune} {via}".strip()
...
async def gestione_vigili_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📥 Importa CSV Vigili", callback_data="importa_vigili")],
        [InlineKeyboardButton("🔙 Indietro", callback_data="admin_indietro")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def gestione_mezzi_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📥 Importa CSV Mezzi", callback_data="importa_mezzi")],
        [InlineKeyboardButton("🔙 Indietro", callback_data="admin_indietro")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def importa_vigili_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    try:
        await query.answer()
    except BadRequest as e:
    ...
    await query.edit_message_text(
        ...
        "Invia un file CSV con i vigili da importare.\n\n"
        "📋 **Formato richiesto:**\n"
        "Nome,Cognome,Qualifica,Grado_Patente,Patente_Nautica,SAF,TPSS,ATP\n\n"
        "📝 **Esempio:**\n"
        "Mario,Rossi,CSV,III,Sì,No,Sì,No\n"
        "Luca,Bianchi,VV,II,No,Sì,No,Sì\n\n"
        "I vigili esistenti con lo stesso nome e cognome verranno aggiornati."
    )
...
async def importa_mezzi_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        "Invia un file CSV con i mezzi da importare.\n\n"
        "📋 **Formato richiesto:**\n"
        "Targa,Tipo\n\n"
        "📝 **Esempio:**\n"
        "AB123CD,APS\n"
        "EF456GH,ABP\n\n"
        "I mezzi esistenti con la stessa targa verranno aggiornati."
    )
...
async def gestisci_progressivo_modifica(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🏘️ Comune", callback_data="campo_comune")],
        [InlineKeyboardButton("📍 Via", callback_data="campo_via")],
    ...
    id_int, rapporto, prog, num_erba, data_uscita, data_rientro, mezzo_targa, mezzo_tipo, capo, autista, comune, via, indirizzo, tipologia, cambio_personale, km_finali, litri_riforniti, created_at = intervento
    ...
    await update.message.reply_text(
        ...
        f"👨‍🚒 Capo: {capo}\n"
        f"🚗 Autista: {autista}\n"
        f"🏘️ Comune: {comune or 'Non specificato'}\n"
        f"📍 Via: {via or 'Non specificata'}\n"
...
async def gestisci_selezione_campo(update: Update, context: ContextTypes.DEFAULT_TYPE, campo: str):
    ...
    if campo in ['mezzo', 'capopartenza', 'autista']:
        ...
        messaggi_campi = {
            'comune': "Inserisci il nuovo comune:",
...
async def gestisci_valore_modifica(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if campo == 'km_finali':
            ...
            nuovo_valore = normalizza_comune(nuovo_valore)
...
async def esporta_dati(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("👥 Esporta Vigili", callback_data="export_vigili")],
        [InlineKeyboardButton("🚒 Esporta Mezzi", callback_data="export_mezzi")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def esegui_export(update: Update, context: ContextTypes.DEFAULT_TYPE, tipo: str, anno: str = None):
    ...
    try:
        if tipo == 'anno':
            ...
            csv_data = esporta_csv_vigili()
            csv_bytes = csv_data.encode('utf-8')
            csv_file = BytesIO(csv_bytes)
            csv_file.name = f"vigili_export_{datetime.now().strftime('%Y%m%d_%H%M')}.csv"
            ...
            await context.bot.send_document(
                ...
                caption="📤 **Esportazione vigili**\n\nFile CSV contenente tutti i vigili."
            )
            return
        elif tipo == 'mezzi':
            csv_data = esporta_csv_mezzi()
            csv_bytes = csv_data.encode('utf-8')
            csv_file = BytesIO(csv_bytes)
            csv_file.name = f"mezzi_export_{datetime.now().strftime('%Y%m%d_%H%M')}.csv"
            ...
            await context.bot.send_document(
                ...
                document=csv_file,
                filename=csv_file.name,
                caption="📤 **Esportazione mezzi**\n\nFile CSV contenente tutti i mezzi."
            )
            return
        else:  # tutto
            csv_data = esporta_csv_interventi()
            csv_bytes = csv_data.encode('utf-8')
            csv_file = BytesIO(csv_bytes)
            csv_file.name = f"interventi_export_completo_{datetime.now().strftime('%Y%m%d_%H%M')}.csv"
...
• ➕ Nuovo Tipo Intervento - Aggiungi nuovo tipo intervento
...
• 📊 Backup automatico giornaliero alle 23:55
• 📅 Backup mensile vigili/mezzi a fine mese
• 🔒 Accesso controllato
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('nuovo_tipo_intervento'):
        await gestisci_nuovo_tipo_intervento(update, context)
        return
    ...
    elif text == "➕ Nuovo Tipo Intervento":
        await avvia_nuovo_tipo_intervento(update, context)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Gestione nuovo tipo intervento
    elif data.startswith("conferma_tipo_"):
        tipo = data.replace('conferma_tipo_', '')
        await conferma_nuovo_tipo_intervento(update, context, tipo)
    
    elif data == "annulla_tipo":
        await query.edit_message_text("❌ Aggiunta tipo intervento annullata.")
        if 'nuovo_tipo_intervento' in context.user_data:
            del context.user_data['nuovo_tipo_intervento']
    ...
    # Gestione admin
        ...
        await importa_vigili_info(update, context)
    
    elif data == "importa_mezzi":
    ...
    # Gestione esportazione
        ...
        await esegui_export(update, context, 'vigili')
    
    elif data == "export_mezzi":
        await esegui_export(update, context, 'mezzi')
        ...
        user_id_rimuovere = int(data[8:])
        rimuovi_utente(user_id_rimuovere)
        
        await query.edit_message_text("✅ Utente rimosso dal sistema.")
...
def main():
    ...
    # Avvia il scheduler per i backup automatici
    asyncio.ensure_future(scheduler_backup_automatico())
    ...
    print("📊 Backup automatici giornalieri alle 23:55")
    print("📅 Backup mensili vigili/mezzi a fine mese")
